		 ...
		 for(var i in robots) {
			 ...
			 do {
				 ...
				 for(var robot in this.robots) {
					 if( robot.line   == l
					   &&robot.column == c) {samePlace = true; break;}
					}
				 if(!samePlace) {break;}
				} while(true);
	...
	, ProcessProposition : function(proposition) {
		 ...
		 for(var i=0; i<proposition.length && state==''; i++) {
			 switch(proposition[i].command) {
				 ...
				 break;
				 default:
					throw new Error({error: 'INVALID_SUBCOMMAND', detail: 'position '+i+' command ' + proposition[i]});
				}
			}
					 ...
					 switch(REST_command) {
						 case 'proposition':
							...
							try {var answer = RRServer.games.ProcessProposition( req.body.idGame, req.body.login, JSON.parse( req.body.proposition ));
								 ...
								 console.log("Error while processing proposition" + err.stack );
								 res.writeHead(400);
